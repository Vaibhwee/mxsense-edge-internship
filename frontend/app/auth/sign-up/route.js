...
import { getAuthRegisterUrl } from "../../../lib/authBackend";
...
export async function POST(request) {
  ...
  if (!username || !password) {
    return redirectWithError(request, "Username and password are required.");
  }

  let res;
  try {
    res = await fetch(getAuthRegisterUrl(), {
      method: "POST",
    ...
    return redirectWithError(
      request,
      "Cannot reach the registration server. Start Django or set BACKEND_API_ORIGIN."
    );
  }
  ...
  if (!access) {
    return redirectWithError(request, "Registration response missing token.");
  }
